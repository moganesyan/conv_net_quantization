...
def optimize_plot(config,session, data, optimizer, accuracy,grads,data_tst,y_pred_cls,data_trn):
    # Ensure we update the global variable rather than a local copy.
    global total_iterations
    global total_trn_error
    #global BITW,BITA,BITG
...
#        print(checker)
    ...
    row_df.to_csv("trn_tst_errors_32.csv")
...
def optimize_gradual(config,session, data, optimizer, accuracy,grads,data_tst,y_pred_cls,data_trn):
    # Ensure we update the global variable rather than a local copy.
    global total_iterations
    global total_trn_error
    #global BITW,BITA,BITG
...
#            print(g_temp[g_nans==True])
    ...
    row_df=pd.DataFrame(row)
    row_df.to_csv("trn_tst_errors_gradual_mode2_2bit.csv")
...
#        print(temp)
        ...
        elif "gradual" in str(FLAGS.mode):
            feed_dict = {x: images,
                 y_true: labels,
                 bitw:32,
                 bita:32,
...
#    print("- Validation-set:\t{}".format(len(data.validation.labels))) 
    ...
    tmp_y=data_trn[1].reshape(50000,1)
    tmp_x=data_trn[0]
...
#    test_writer = tf.summary.FileWriter(FLAGS.summaries_dir + '/test')
    ...
    if "normal" in str(FLAGS.mode):
        ...
        optimize_plot(config=model,session=session,data=data_trn,optimizer=optimizer,
                  accuracy=accuracy,grads=gvs,data_tst=data_tst,y_pred_cls=y_pred_cls,data_trn=data_trn)
        
        
    elif "gradual" in str(FLAGS.mode):
        optimize_gradual(config=model,session=session,data=data_trn,optimizer=optimizer,
                accuracy=accuracy,grads=gvs,data_tst=data_tst,y_pred_cls=y_pred_cls,data_trn=data_trn)
