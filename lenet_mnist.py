...
import math
from dorefa3 import get_dorefa
import dorefa3
...
def new_conv_layer(x_in, name,             # The previous layer.
    ...
    if "conv1" in name:
        ...
        with tf.variable_scope(name,reuse=tf.AUTO_REUSE),dorefa3.replace_variable(
                    lambda x: fw(x)):
...
def new_fc_layer(x_in,name,          # The previous layer.
                 ...
                 bitw): # Use Rectified Linear Unit (ReLU)?
    global fw
...
#layer_flat=tf.Print(layer_flat,[layer_flat],message="Flat layer:",summarize=10)
#print("Passed flattening ")


#FC layer 1
layer_fc1,weights_fc1 = new_fc_layer(x_in=layer_flat,name="fc1",
...
#layer_fc1=fg(layer_fc1)
#layer_fc1=tf.Print(layer_fc1,[weights_fc1],message="weights fc1",summarize=10)
#layer_fc1=tf.Print(layer_fc1,[layer_fc1],message="fc1",summarize=10)
...
#layer_fc1=tf.Print(layer_fc1,[layer_fc1],message="after cabs fc1",summarize=sum_size)

layer_fc1=fa(layer_fc1)
#layer_fc1=tf.Print(layer_fc1,[layer_fc1],message="after act fc1",summarize=sum_size)
...
#Output FC layer
layer_fc2,weights_fc2 = new_fc_layer(x_in=layer_fc1,name="fco",
                         num_inputs=fc_size,
...
def plot_conv_weights(weights, input_channel=0):
    ...
    w = session.run(weights, feed_dict={bitw:BITW,bita:BITA,bitg:BITG,phase_train:True})
