...
#                lambda x: fw(x)):
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE),dorefa.replace_variable(
                lambda x: dorefa.quantize_w(x,bit=bitw)):
...
layer_conv2, weights_conv2 = \
    new_conv_layer(input=layer_conv1_print,name="conv2",
...
def plot_conv_weights(weights, input_channel=0):
    ...
    w = session.run(weights, feed_dict={bitw:BITW})
...
print("conv2 weights")
print(weights_conv2)
print(session.run(weights_conv2, feed_dict={bitw:1}))
