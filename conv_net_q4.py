...
def new_conv_layer(input, name,             # The previous layer.
...
#    biases=tf.get_variable('b',initializer=B_init)
    ...
    #weights_print=tf.Print(weights_quant,[weights_quant],message="The weights of {} are".format(weights.name))
    layer = tf.nn.conv2d(input=input,
                         filter=weights_quant,
                         strides=[1, 1, 1, 1],
                         padding='SAME')
...
def new_fc_layer(input,name,          # The previous layer.
...
#    print(biases.name)
    ...
    layer = tf.matmul(input, weights_quant) + biases
...
layer_conv1, weights_conv1 = \
    new_conv_layer(input=x_image,name="conv1",
...
layer_conv2, weights_conv2 = \
    new_conv_layer(input=layer_conv1,name="conv2",
...
layer_flat, num_features = flatten_layer(layer_conv2_act)
...
#Output FC layer
layer_fc2 = new_fc_layer(input=layer_fc1,name="fco",
                         num_inputs=fc_size,
...
#plot_conv_weights(weights=fw(weights_conv1))
